 ...
 * variables (optarg_, optind, optopt).
 *
...
struct optparse {
    ...
    int optopt;
    char * optarg_;
    char errmsg[optparse_errmsg_count];
    int subopt;
};
...
optparse(struct optparse *options, const char *optstring)
{
    ...
    case OPTPARSE_REQUIRED:
        ...
        options->optind++;
        if (option[1]) {
            options->optarg_ = option + 1;
        } else if (next != 0) {
            options->optarg_ = next;
            options->optind++;
        } else {
            char str[2] = {0, 0};
            str[0] = option[0];
    ...
    for (i = 0; !optparse_longopts_end(longopts, i); i++) {
        ...
        if (optparse_longopts_match(name, option)) {
            ...
            if (longopts[i].argtype == OPTPARSE_NONE && arg != 0) {
                ...
                options->optarg_ = options->argv[options->optind];
                if (options->optarg_ == 0)
